...
<div class="row">
    <div class="col-lg-12 mx-auto">
        <div class="progress my-shadow progress-bar-main">
            <div
                class="progress-bar progress-bar-<?= ViewHelper::getLevelColorClass($model['level']) ?> progress-bar-striped active"
                role="progressbar"
                style="width: <?= ViewHelper::getProgress($model['level']) ?>%"><?= $model['level'] ?></div>
        </div>
    </div>
    <div class="col-lg-3 mx-auto">
        <div class="card">
            ...
            <div class="card-body" <?= ViewHelper::friendOnline($model) ?>>
                ...
                <p class="card-text"><b><?= Yii::t('app', 'Phone:') ?></b> <a class="phone"
                ...
                <p class="card-text"><b><?= Yii::t('app', 'Correction Point') ?>:</b> <?= $model['correction_point'] ?>
                </p>
                <p class="card-text"><b><?= Yii::t('app', 'Achievements:') ?></b> <?= $model['howach'] ?></p>
                <p class="card-text"><b><?= Yii::t('app', 'Host') ?>:</b> <?= $model['location'] ?></p>
                <p class="card-text"><b><?= Yii::t('app', 'Last login') ?>:</b> <?php if ($model['lastloc'] == 0) {
                        echo Yii::t('app', 'ONLINE');
                    } else {
                        echo ViewHelper::getHumanTime($model['lastloc']);
                    } ?></p>
                <p class="card-text"><b><?= Yii::t('app', 'Hours at cluster') ?>:</b> <?= $model['hours'] ?></p>
                <p class="card-text"><a href="//profile.intra.42.fr/users/<?= $model['login'] ?>" target="_blank"
                                        class="btn btn-warning bg-warning">Intra</a></p>
                <p class="card-text"><a href="<?= '/' . Yii::$app->language . '/' . $switch ?>/<?= $model['login'] ?>"
                                        class="btn btn-success bg-success"><?= ucfirst(substr_replace($switch, "", -1)) ?>
                        Profile</a></p>
                <?php if ($model['login'] !== Yii::$app->user->identity->username) {
                    if (\app\models\Friend::check($model['login'])) { ?>
                        <a href="<?php echo '/' . Yii::$app->language ?>/friends/delete/<?= $model['login'] ?>"
                           id="friends-delete" class="btn btn-md btn-danger bg-danger"
                           data-method="POST"><?= Yii::t('app', 'Delete Friend') ?></a>
                    <?php } else {
                        ; ?>
                        <a href="<?php echo '/'
                                            . Yii::$app->language ?>/friends/create/<?= $model['login'] ?>/<?= $explode ?>"
                           id="friends-create"
                           class="btn btn-md btn-success bg-success"><?= Yii::t('app', 'Add Friend') ?></a>
                    <?php }
                } ?>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                ...
                <?php foreach ($skills as $skill) { ?>
                    <div class="progress" data-placement="left" data-toggle="tooltip"
                         title="<?= $skill['skills_level'] ?>">
                        <div
                            class="progress-bar mini progress-bar-<?= ViewHelper::getLevelColorClass($skill['skills_level']) ?> progress-bar-striped active"
                            role="progressbar" style="width:<?= ViewHelper::getProgress($skill['skills_level']) ?>%">
    ...
    $friends =
        $this->render('@app/views/friend/view', ['searchModel' => $searchModelFriend,
            'dataProvider' => $dataProviderFriend,
            'tab' => 'profile']);
