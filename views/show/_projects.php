    ...
    <table class="table ">
                ...
                <tr>
                    <td>
                        <div class="progress my-shadow">
                            <div
                                class="progress-bar progress-bar-<?= ViewHelper::getProgressProjectColor($item->final_mark, $course, $item->status) ?>"
                                role="progressbar"
                                style="width:<?= ViewHelper::getProgressProject($item->final_mark, $course) ?>%">
                                <p><a style="color:white; " href="<?= '/'
                                                                      . Yii::$app->language
                                                                      . $urlHelperForProjects ?><?= $item->slug ?>"><?= $item->name ?></a>
                                </p>
                            </div>
                        </div>
                    </td>
        ...
        <?php if (isset($parents)) {
            foreach ($parents as $parent => $value) { ?>
                <tr>
                    <td colspan="4"><h5 class="card-title"><?= $parent ?></h5></td>
                </tr>
                <?php if (isset($value)) {
